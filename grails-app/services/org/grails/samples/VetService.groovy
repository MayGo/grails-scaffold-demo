...
class VetService{
  ...
  List parseParamsAndRetrieveListAndCount(Map param){
		...
		def persistentProperties = new DefaultGrailsDomainClass(Vet.class).persistentProperties

		def persistentPropertiesMap = [:] as HashMap
		persistentProperties.each {
			persistentPropertiesMap.put(it.name, it)
		}
		def	results = Vet.createCriteria().list(offset:param.offset, max:param.max, order:param.order, sort:param.sort) {

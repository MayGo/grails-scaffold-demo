...
@Transactional
class DivisionCollectionService{
  ...
  List parseParamsAndRetrieveListAndCount(Map param){
		...
		def persistentProperties = new DefaultGrailsDomainClass(DivisionCollection.class).persistentProperties

		def persistentPropertiesMap = [:] as HashMap
		persistentProperties.each {
			persistentPropertiesMap.put(it.name, it)
		}
		def	results = DivisionCollection.createCriteria().list(offset:param.offset, max:param.max, order:param.order, sort:param.sort) {

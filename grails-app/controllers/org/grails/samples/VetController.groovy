...
class VetController extends CustomRestfulController<Vet>{
	...
	@RestApiParams(params=[
		@RestApiParam(name="max", type="long", paramType = RestApiParamType.QUERY, description = "Max number of Vet to retrieve"),
		@RestApiParam(name="offset", type="long", paramType = RestApiParamType.QUERY, description = "Retrieved Vet list offset"),
		@RestApiParam(name="order", type="string", paramType = RestApiParamType.QUERY, description = "Retrieved Vet list order",allowedvalues=['asc','desc']),
		@RestApiParam(name="sort", type="string", paramType = RestApiParamType.QUERY, description = "Retrieved Vet list sort")
	])
	def index(final Integer max) {
		...
		// This is here so running demo works right away. Should be replaced with own service, eg: vetService.list(params)
		List results = vetService.parseParamsAndRetrieveListAndCount(params) 

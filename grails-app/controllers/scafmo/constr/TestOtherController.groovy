...
class TestOtherController extends CustomRestfulController<TestOther>{
	...
	@RestApiParams(params=[
		@RestApiParam(name="max", type="long", paramType = RestApiParamType.QUERY, description = "Max number of TestOther to retrieve"),
		@RestApiParam(name="offset", type="long", paramType = RestApiParamType.QUERY, description = "Retrieved TestOther list offset"),
		@RestApiParam(name="order", type="string", paramType = RestApiParamType.QUERY, description = "Retrieved TestOther list order",allowedvalues=['asc','desc']),
		@RestApiParam(name="sort", type="string", paramType = RestApiParamType.QUERY, description = "Retrieved TestOther list sort")
	])
	def index(final Integer max) {
		...
		// This is here so running demo works right away. Should be replaced with own service, eg: testOtherService.list(params)
		List results = testOtherService.parseParamsAndRetrieveListAndCount(params) 

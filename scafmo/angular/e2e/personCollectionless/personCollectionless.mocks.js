module.exports = function(){
	angular.module('httpBackendMock', ['ngMockE2E'])
		.run(function($httpBackend, appConfig) {
			console.log("running module personCollectionlessHttpBackendMock")
			function quote(str) {
				return str.replace(/(?=[\/\\^$*+?.()|{}[\]])/g, "\\");
			}

			var url = appConfig.restUrl + '/personcollectionlesss/v1';
			...
			var divisionUrl =/.*\/divisioncollectionlesss\/v1.*/;
			$httpBackend.whenGET(divisionUrl).respond(function(method, url){return [200, [{'id': '1','name': 'Division152'}]]});//list

...
class PersonCollectionlessController extends CustomRestfulController<PersonCollectionless>{
    ...
    PersonCollectionlessService personCollectionlessService
	...
	@RestApiParams(params=[
		@RestApiParam(name="max", type="long", paramType = RestApiParamType.QUERY, description = "Max number of PersonCollectionless to retrieve"),
		@RestApiParam(name="offset", type="long", paramType = RestApiParamType.QUERY, description = "Retrieved PersonCollectionless list offset"),
		@RestApiParam(name="order", type="string", paramType = RestApiParamType.QUERY, description = "Retrieved PersonCollectionless list order",allowedvalues=['asc','desc']),
		@RestApiParam(name="sort", type="string", paramType = RestApiParamType.QUERY, description = "Retrieved PersonCollectionless list sort")
	])
	def index(final Integer max) {
		...
		// This is here so running demo works right away. Should be replaced with own service, eg: personCollectionlessService.list(params)
		List results = personCollectionlessService.parseParamsAndRetrieveListAndCount(params) 

...
class OwnerService{
  ...
  List parseParamsAndRetrieveListAndCount(Map param){
		...
		def persistentProperties = new DefaultGrailsDomainClass(Owner.class).persistentProperties

		def persistentPropertiesMap = [:] as HashMap
		persistentProperties.each {
			persistentPropertiesMap.put(it.name, it)
		}
		def	results = Owner.createCriteria().list(offset:param.offset, max:param.max, order:param.order, sort:param.sort) {

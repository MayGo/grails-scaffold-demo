...
class TestStringController extends CustomRestfulController<TestString>{
	...
	@RestApiParams(params=[
		@RestApiParam(name="max", type="long", paramType = RestApiParamType.QUERY, description = "Max number of TestString to retrieve"),
		@RestApiParam(name="offset", type="long", paramType = RestApiParamType.QUERY, description = "Retrieved TestString list offset"),
		@RestApiParam(name="order", type="string", paramType = RestApiParamType.QUERY, description = "Retrieved TestString list order",allowedvalues=['asc','desc']),
		@RestApiParam(name="sort", type="string", paramType = RestApiParamType.QUERY, description = "Retrieved TestString list sort")
	])
	def index(final Integer max) {
		...
		// This is here so running demo works right away. Should be replaced with own service, eg: testStringService.list(params)
		List results = testStringService.parseParamsAndRetrieveListAndCount(params) 

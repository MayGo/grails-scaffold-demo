...
class TestStringService{
  ...
  List parseParamsAndRetrieveListAndCount(Map param){
		...
		def persistentProperties = new DefaultGrailsDomainClass(TestString.class).persistentProperties

		def persistentPropertiesMap = [:] as HashMap
		persistentProperties.each {
			persistentPropertiesMap.put(it.name, it)
		}
		def	results = TestString.createCriteria().list(offset:param.offset, max:param.max, order:param.order, sort:param.sort) {

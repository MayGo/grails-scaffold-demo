...
class PersonService{
  ...
  List parseParamsAndRetrieveListAndCount(Map param){
		...
		def persistentProperties = new DefaultGrailsDomainClass(Person.class).persistentProperties

		def persistentPropertiesMap = [:] as HashMap
		persistentProperties.each {
			persistentPropertiesMap.put(it.name, it)
		}
		def	results = Person.createCriteria().list(offset:param.offset, max:param.max, order:param.order, sort:param.sort) {

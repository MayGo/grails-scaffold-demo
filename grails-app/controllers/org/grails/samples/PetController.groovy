...
class PetController extends CustomRestfulController<Pet>{
	...
	@RestApiParams(params=[
		@RestApiParam(name="max", type="long", paramType = RestApiParamType.QUERY, description = "Max number of Pet to retrieve"),
		@RestApiParam(name="offset", type="long", paramType = RestApiParamType.QUERY, description = "Retrieved Pet list offset"),
		@RestApiParam(name="order", type="string", paramType = RestApiParamType.QUERY, description = "Retrieved Pet list order",allowedvalues=['asc','desc']),
		@RestApiParam(name="sort", type="string", paramType = RestApiParamType.QUERY, description = "Retrieved Pet list sort")
	])
	def index(final Integer max) {
		...
		// This is here so running demo works right away. Should be replaced with own service, eg: petService.list(params)
		List results = petService.parseParamsAndRetrieveListAndCount(params) 
